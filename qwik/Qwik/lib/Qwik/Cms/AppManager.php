...
use Qwik\Component\Routing\Router;
...
class AppManager {
    ...
    /**
     * @var \Qwik\Cms\Site\Site Site en cours
     */
    private $site;
    /**
     * @var \Qwik\Component\Routing\Router
     */
    private $router;
    /**
     ...
     * @param string $domain Nom de domaine (peut avoir un dev.) devant
     * @return string
     */
    ...
    /**
     * @param \Qwik\Environment\Environment $environment
     */
    ...
    public function init($www, $domain){
        ...
        $this->router = new Router($this->getBaseUrl());
    ...
    /**
     * @return \Qwik\Component\Routing\Router
     */
    ...
    /**
     * @param \Exception $ex
     * @return \Qwik\Component\Routing\Response
     */
    private function getResponseForException(\Exception $ex){
        ...
        if(!is_null($page)){
            ...
            Logger::getInstance()->warning('<h1>Error '.$ex->getCode(). '</h1>' . $ex->getMessage().' ('.$ex->getFile().'. Line:'.$ex->getLine() .')');
        }
